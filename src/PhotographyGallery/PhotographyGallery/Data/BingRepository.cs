...
namespace PhotographyGallery.Data
{
    public class BingRepository
    {
         public List<BingImage> GetBingImages()
        {
            ...
            byte[] searchResults = webClient.DownloadData(string.Format("https://api.cognitive.microsoft.com/bing/v5.0/images/search?q={0}&mkt=en-us", "awesome+pictures&count=5"));
            var serializer = new JsonSerializer();
            using (var stream = new MemoryStream(searchResults))
            using (var reader = new StreamReader(stream))
            using (var jsonReader = new JsonTextReader(reader))
            {
                return results = serializer.Deserialize<BingImageResponse>(jsonReader).value.ToList();
            }
        }
    }
}